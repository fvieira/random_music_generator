import abjad
from abjad import Rest, Note, Staff, Score, Measure, show
...
DURATIONS = [('1', F(1)), ('2', F(3)), ('4', F(5)), ('8', F(5))]
...
def generate_random_measure(measure_size=4, notes=None, durations=None):
    ...
    while measure_space_left > 0:
        ...
        duration = pick_random_duration(durations)[0]
        if random.random() < REST_PROBABILITY:
            note = Rest('r' + duration)
        else:
            note = Note(random.choice(NOTES) + duration)
        notes.append(note)
        measure_space_left -= F(1, int(duration))
...
def filter_duration_probabilities(durations, space_left):
    ...
    for duration, probability in durations:
        if F(1, int(duration)) <= space_left:
