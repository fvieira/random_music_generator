...
class Bottle(object):
    ...
    def wsgi(self, environ, start_response):
        ...
        # FIXME Huge hack to get CORS working
        def cors_start_response(message, headers, *args, **kwargs):
            headers.append(('Access-Control-Allow-Origin', '*'))
            headers.append(('Access-Control-Allow-Methods', 'GET, POST, PUT, OPTIONS'))
            headers.append(('Access-Control-Allow-Headers', 'Origin, Accept, Content-Type, X-Requested-With, X-CSRF-Token'))
            return start_response(message, headers, *args, **kwargs)
        ...
        try:
            ...
            or environ['REQUEST_METHOD'] == 'HEAD':
                ...
                out = []
            cors_start_response(response._status_line, response.headerlist)
            return out
        except (KeyboardInterrupt, SystemExit, MemoryError):
            raise
        except Exception:
            ...
            cors_start_response('500 INTERNAL SERVER ERROR', headers, sys.exc_info())
            return [tob(err)]
